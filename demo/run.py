...
EXT = sys.argv[1] if len(sys.argv) > 1 else "./build/debug/semantic_profile.duckdb_extension"
...
show("Dry run: what this will cost before anything is spent",
     f"SELECT n_columns, rows_to_probe, total_requests, approx_input_tokens FROM sem_cost('shipments', rows:={ROWS})")
...
show("Step 2 — which checks this table warrants (top 15 of the selected set)",
     """SELECT scope, column_name, probe_id, round(relevance,2) AS relevance
        FROM sem_probes('shipments') WHERE selected ORDER BY relevance DESC""", 15)
...
show("Step 3 — findings, with evidence",
     f"""SELECT scope, column_name, probe_id, rows_flagged, flag_rate, needs_review,
         ...
         FROM sem_report('shipments', rows:={ROWS})""")
...
show("Cross-column contradictions, per row",
     f"""SELECT row_id, probe_id, probability,
         ...
         FROM sem_findings('shipments', rows:={ROWS}) WHERE scope = 'row'""")

show("Everything, one call", f"SELECT * FROM sem_profile('shipments', rows:={ROWS})")

print("\nusage:", con.sql("SELECT sem_stats()").fetchone()[0])
