...
--
-- This is the step that makes the semantic_profile adapt rather than run a fixed list.
...
CREATE OR REPLACE MACRO sem_render(template, column_name, semantic_type) AS
...
-- Repeat calls to sem_columns cost no API requests: the responses are cached,
-- so this is a few hashes and a hash-map lookup per column.
CREATE OR REPLACE MACRO sem_candidates(tbl, n := 200, min_type_confidence := 0.6) AS TABLE
    SELECT c.column_name, c.semantic_type, c.role, c.sample_values,
           p.probe_id,
           sem_render(p.instructions, c.column_name, c.semantic_type)   AS instructions,
           json(sem_render(p.criteria, c.column_name, c.semantic_type)) AS criteria
    FROM sem_columns(tbl, n) c, sem_catalog() p
    WHERE p.scope = 'value'
...
CREATE OR REPLACE MACRO sem_selection_question(instructions) AS q_noul(
    json_object(
        'check',    instructions,
...
CREATE OR REPLACE MACRO sem_value_probes(tbl, n := 200, min_type_confidence := 0.6) AS TABLE
    WITH cand AS (SELECT * FROM sem_candidates(tbl, n, min_type_confidence)),
    asked AS (
        SELECT column_name,
               ts_answers(
                   json_object(
                   ...
                   json_group_object(probe_id, sem_selection_question(instructions))) AS a
        FROM cand
...
CREATE OR REPLACE MACRO sem_row_probes(tbl, n := 200) AS TABLE
    WITH cat AS (SELECT * FROM sem_catalog() WHERE scope = 'row'),
    asked AS (
        SELECT ts_answers(
                   json_object(
                       'table',        tbl,
                       'columns',      (SELECT to_json(list(struct_pack(
                                            ...
                                            role := role)))
                                        FROM sem_columns(tbl, n)),
                       'example_rows', json(sem_context_rows(tbl, 5))),
...
-- The public selection view. `selected` is exactly what sem_values executes.
CREATE OR REPLACE MACRO sem_probes(
    tbl, n := 200, min_relevance := 0.5, max_per_column := 4, max_row_probes := 5,
    min_type_confidence := 0.6
) AS TABLE
    WITH all_probes AS (
        SELECT * FROM sem_value_probes(tbl, n, min_type_confidence)
        UNION ALL
        SELECT * FROM sem_row_probes(tbl, n)
    ),
    ranked AS (
