...
CREATE OR REPLACE MACRO profile_schema(tbl) AS TABLE
    SELECT cid AS ordinal, name AS column_name, type AS declared_type
    FROM pragma_table_info(tbl);

CREATE OR REPLACE MACRO profile_sample(tbl, n := 200) AS TABLE
...
CREATE OR REPLACE MACRO profile_path(column_name) AS
    '$."' || replace(column_name, '"', '\"') || '"';
...
CREATE OR REPLACE MACRO profile_column_values(tbl, n := 200, per_column := 40) AS TABLE
    WITH samp AS (SELECT * FROM profile_sample(tbl, n)),
    pairs AS (
        SELECT s.ordinal, s.column_name, s.declared_type,
               json_extract(r.row_json, profile_path(s.column_name)) AS val,
               min(r.row_id) AS first_row
        FROM profile_schema(tbl) s, samp r
...
CREATE OR REPLACE MACRO profile_context_rows(tbl, k := 3) AS
    (SELECT json_group_array(json(row_json)) FROM (SELECT row_json FROM profile_sample(tbl, 200) LIMIT k));
