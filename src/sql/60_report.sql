...
-- and `example_rows` the row ids, because a profiler that says "3% of this column
...
CREATE OR REPLACE MACRO profile_column_flags(tbl, n := 200, threshold := 0.7) AS TABLE
    WITH c AS (SELECT * FROM profile_columns(tbl, n))
    SELECT * FROM (
...
CREATE OR REPLACE MACRO profile_report(
    tbl, threshold := 0.7, rows := 100, n := 200,
    min_relevance := 0.5, max_per_column := 4, max_row_probes := 5, min_type_confidence := 0.6
) AS TABLE
    WITH v AS (
        SELECT * FROM profile_values(tbl, rows, n, min_relevance, max_per_column, max_row_probes, min_type_confidence)
    )
    SELECT scope,
           column_name,
           probe_id,
...
CREATE OR REPLACE MACRO profile_findings(
    tbl, threshold := 0.7, rows := 100, n := 200,
    min_relevance := 0.5, max_per_column := 4, max_row_probes := 5, min_type_confidence := 0.6
) AS TABLE
    SELECT row_id, scope, column_name, probe_id, round(probability, 3) AS probability, value, row_json
    FROM profile_values(tbl, rows, n, min_relevance, max_per_column, max_row_probes, min_type_confidence)
    WHERE probability >= threshold
    ORDER BY probability DESC, row_id;

-- Dry run. Makes no API calls, so this is safe to run before committing spend.
CREATE OR REPLACE MACRO profile_cost(tbl, rows := 100, n := 200) AS TABLE
    WITH s AS (SELECT count(*) AS n_columns FROM profile_schema(tbl)),
    t AS (SELECT count(*) AS table_rows FROM query_table(tbl)),
    b AS (SELECT avg(length(row_json)) AS avg_row_bytes FROM profile_sample(tbl, least(50, n))),
    e AS (
...
CREATE OR REPLACE MACRO profile(tbl, threshold := 0.7, rows := 100, n := 200) AS TABLE
    SELECT * FROM (
